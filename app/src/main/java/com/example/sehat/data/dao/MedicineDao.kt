...
interface MedicineDao {
    @Query("SELECT * FROM medicines GROUP BY name ORDER BY name ASC")
    fun getAllMedicines(): Flow<List<Medicine>>

    @Query("SELECT * FROM medicines WHERE (name LIKE '%' || :query || '%' OR form LIKE '%' || :query || '%') GROUP BY name ORDER BY name ASC")
    fun searchMedicines(query: String): Flow<List<Medicine>>

    @Query("SELECT * FROM medicines WHERE name = :medicineName ORDER BY facility ASC")
    fun getFacilitiesForMedicine(medicineName: String): Flow<List<Medicine>>

    @Query("SELECT * FROM medicines ORDER BY facility ASC")
    fun getAllFacilityStocks(): Flow<List<Medicine>>
